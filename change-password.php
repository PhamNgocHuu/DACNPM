...
if (isset($_POST['submit'])) {
     ...
     */
    $query = "SELECT * FROM taikhoan WHERE id = '{'$id'}' AND matkhau = '{$matkhau}'";
    ...
    if (mysqli_num_rows($result) > 0) { // mysqli_num_rows: kiểm tra (đếm) có bao nhiêu bản ghi (rows)
        ...
        $dt = date("Y-m-d H:i:s");
        $query = "UPDATE taikhoan SET matkhau = '{$matkhau}', email = '{$email}' WHERE id = '{'$id'}'";
        $result = mysqli_query($connect, $query);
        if ($result) {
            $success = "Sửa mật khẩu thành công";
        } else {
            $errors[] = "Sửa mật khẩu thất bại: " . mysqli_error($connect); // mysqli_error: là cú pháp hiện lỗi của mysqli
        }
    } else {
        // Đăng nhập thất bại
